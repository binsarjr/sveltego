...
func TestValidateRunesOption_optOutWithRune(t *testing.T) {
	t.Parallel()
	f := false
	opts := svelteOptions{Present: true, Runes: &f, Pos: ast.Pos{Line: 1, Col: 1}}
	scripts := scriptOutput{Runes: []string{"$state"}}
	err := validateRunesOption(opts, scripts)
	if err == nil {
		t.Fatal("expected runes opt-out mismatch")
	}
	if !strings.Contains(err.Error(), "$state") {
		t.Errorf("err missing rune name: %v", err)
	}
}

func TestValidateRunesOption_optOutNoRune(t *testing.T) {
	t.Parallel()
	f := false
	opts := svelteOptions{Present: true, Runes: &f}
	if err := validateRunesOption(opts, scriptOutput{}); err != nil {
		t.Fatalf("clean opt-out should pass: %v", err)
	}
}

func TestValidateRunesOption_optInWithRune(t *testing.T) {
	t.Parallel()
	tr := true
	opts := svelteOptions{Present: true, Runes: &tr}
	scripts := scriptOutput{Runes: []string{"$state"}}
	if err := validateRunesOption(opts, scripts); err != nil {
		t.Fatalf("opt-in with rune should pass: %v", err)
	}
}

func TestValidateRunesOption_absent(t *testing.T) {
	t.Parallel()
	if err := validateRunesOption(svelteOptions{}, scriptOutput{Runes: []string{"$state"}}); err != nil {
		t.Fatalf("absent option should not fail: %v", err)
	}
}

...
func emitSSRWire(genRoot, modulePath string, route mirrorRoute) error {
	...
	b.Line(")")
	b.Line("")
	b.Line("// RenderSSR wraps the typed usersrc.Render(payload, PageData,")
	b.Line("// pageState) so the manifest's PageHandler adapter can dispatch")
	b.Line("// without a per-route generic. A nil data value zero-values")
	b.Line("// PageData; a type-mismatch surfaces as a descriptive error")
	b.Line("// instead of a panic. pageState carries the $app/state surface")
	b.Line("// (URL, params, route, status, error, data, form, state) for")
	b.Line("// templates that read the runes (issue #466).")
	b.Line("func RenderSSR(payload *server.Payload, data any, pageState server.PageState) error {")
	b.Indent()
	...
	b.Dedent()
	b.Line("}")
	b.Line("usersrc.Render(payload, typed, pageState)")
	b.Line("return nil")
	b.Dedent()
	b.Line("}")
...
func emitSSRLayoutWire(genRoot, modulePath string, route mirrorRoute) error {
	...
	b.Line("// children, pageState) so the manifest's LayoutHandler bridge can")
	b.Line("// dispatch the layout chain via *server.Payload callbacks (issue")
	b.Line("// #440). A nil data value zero-values LayoutData; a type-mismatch")
	b.Line("// surfaces as a descriptive error instead of a panic. inner may be")
	b.Line("// nil — layouts that don't render content past their chrome simply")
	b.Line("// skip the call. pageState carries the $app/state surface for")
	b.Line("// layouts that read the runes (issue #466).")
	b.Line("func RenderLayoutSSR(payload *server.Payload, data any, inner func(*server.Payload), pageState server.PageState) error {")
	b.Indent()
	...
	b.Dedent()
	b.Line("}")
	b.Line("usersrc.Render(payload, typed, inner, pageState)")
	b.Line("return nil")
	b.Dedent()
	b.Line("}")
...
func emitSSRErrorWire(genRoot, modulePath string, route mirrorRoute) error {
	...
	b.Line("// pageState) so the manifest's payload-bridge ErrorHandler adapter")
	b.Line("// can dispatch the boundary via *server.Payload (issue #412).")
	b.Line("// kit.SafeError is the fixed data shape for every error template; no")
	b.Line("// per-route generic. pageState carries the $app/state surface for")
	b.Line("// error templates that read the runes (issue #466).")
	b.Line("func RenderErrorSSR(payload *server.Payload, safe kit.SafeError, pageState server.PageState) {")
	b.Indent()
	b.Line("errorsrc.Render(payload, safe, pageState)")
	b.Dedent()
	b.Line("}")

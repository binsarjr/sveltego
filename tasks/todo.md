...
- [ ] Phase 0j-fix — close #109 (codegen wire emits `PageData(raw)` value conversion) and #110 (pre-commit hook skips user `src/routes/`, `src/params/`, `hooks.server.go`); land the deferred user `.go` files; flip `PLAYGROUND_SMOKE=enabled` and confirm `/` + `/post/123` return 200 with expected markers. Closes #23, MVP scope CLOSED.
- [ ] v0.2 backlog — layouts (#24), hooks (#25), error boundaries (#26), form actions (#27–#29), route groups (#30), page options (#31, #80), env (#32, #33), `kit.Cookies/Redirect/Fail/Error` (#78, #79, #81, #82). 15 issues total.

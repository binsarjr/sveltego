...
Pre-alpha. MVP, v0.2, v0.3, v0.4, and v1.1 closed; v0.5, v0.6, and v1.0 in flight (v0.5: 4 open / 19 closed; v0.6: 31 open / 9 closed; v1.0: 3 open / 59 closed). The SSR Option B track ([RFC #421](https://github.com/binsarjr/sveltego/issues/421); 9 phases #423–#431) shipped 2026-05-02 — SSR is live end-to-end via build-time JS-to-Go transpile (no JS engine on the request path). The Mustache-Go template emitter was atomically deleted via [#486](https://github.com/binsarjr/sveltego/issues/486); only the pure-Svelte + transpiled-SSR path remains. The Go workspace already hosts the core (`packages/sveltego` — including the `internal/codegen/svelte_js2go/` transpiler and `runtime/svelte/{server,fallback}` helpers), auth (`packages/auth`, `packages/cookiesession`), tooling (`packages/lsp`, `packages/mcp`, `packages/init`, `packages/enhanced-img`), six deploy adapters plus `adapter-auto`, the bench harness (`bench/`, `benchmarks/`), AI templates (`templates/ai/`), and end-to-end playgrounds (`basic`, `blog`, `dashboard`, `ssr-stress`, `static`). New work continues to flow through:
...
| v1.0 | 62 (#60–69, #89, #91–93, post-merge code-quality follow-ups, SSR Option B track #421/#422 + #423–#431, Mustache-Go atomic delete + post-merge follow-ups #486/#491/#494/#502, hydration spike #315/#503) | Bench, docs, streaming, SSG, CSP, sitemap, image opt, deploy adapters, SSR Option B (RFC #421 + 9 phases) |

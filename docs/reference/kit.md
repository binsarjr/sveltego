...
|---|---|---|
| `RequestEvent` | hooks, `_server.go`, actions | Request-scoped state with `URL`, `Params`, `Locals`, `Cookies`. |
| `RenderCtx` | generated render code | SSR-time context passed into templates. |
| `LoadCtx` | `Load` in `_page.server.go`, `_layout.server.go` | Load-time context with `Parent()` for layout chain. |
